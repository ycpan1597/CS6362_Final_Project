...
def main(argv):
    ...
    if dataset == 'celeba':
        ...
        batch_size = 1000               # The EM batch size
        num_iterations = 30             # Number of EM iterations (=epochs) (default = 30)
        feature_sampling = False       # For faster responsibilities calculation, randomly sample the coordinates (or False)
        mfa_sgd_epochs = 0              # Perform additional training with diagonal (per-pixel) covariance, using SGD (default = 0)
    ...
    model_name = 'c_{}_l_{}_init_{}_run2'.format(n_components, n_factors, init_method)
